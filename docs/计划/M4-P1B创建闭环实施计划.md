...
> **校准说明（2026-08-04）：** 创建流程已定稿为单候选流程（生成 1 个候选 → 满意/重新生成/放弃）；本文档原“4 候选 / 候选网格 / 多候选”相关表述已按定稿流程校准。
...
- 前端：`src/creation/` 创建流程页面（settings 窗口扩展为多页：上传/特征 → 生成进度 → 候选预览/确认（满意/重新生成/放弃））
...
└─ settings.ts（扩展：创建入口 + 候选预览/确认 UI）
```
...
export class CreationFlow {
  ...
  submitBatch(count: number): Promise<void>; // 定稿流程固定生成 1 个候选（调用 gen_start ×1；count 参数保留供测试）
  poll(): Promise<JobUpdate[]>;
  accept(variantId: string): Promise<void>; // 标记 accepted
  compile(): Promise<CompileResult>;        // 编译为运行资产
}
```
- UI：上传照片 → 可选特征（默认留空参考图模式）→ 生成进度（单卡片进度）→ 候选预览（去背景）→ 确认 / 重新生成 / 放弃 → "制作完成，出现在桌面"。
...
| 编号 | 场景 | 预期 |
|---|---|---|
| C-01 | 创建闭环 | 上传照片→生成 1 个候选→确认→桌面出现宠物 |
| C-02 | 候选确认 | 预览满意→编译上线，宠物出现在桌面 |
| C-03 | 抠图质量 | 无过度抠图；质量不合格进入降级/校准 |
| C-04 | 任务进度/取消 | 进度显示、可取消、重启恢复 |
| C-05 | 多宠切换 | 保存/切换/单宠激活 |
| C-06 | 断网 | 已有宠物完全离线可用；生成提示失败 |
| C-07 | 隐私提示 | 上传前明示照片生命周期 |
| C-08 | 资产损坏 | 不阻塞启动（复用占位） |
| C-09 | 重新生成 | 同一宠物再生成新候选，不重复建宠物 |
| C-10 | 放弃创建 | 删除宠物记录并清理任务产物 |

**验收闸门：** C-01~C-10 通过；创建失败不破坏已有宠物；生成完成后完全离线可用；性能无回归（companion 采样）。

- [ ] **Step 1: 创建 M4 检查脚本与人工清单。**
- [ ] **Step 2: 全量闸门 + companion 性能回归。**
- [ ] **Step 3: 用户实测 C-01~C-10，回填清单。**
- [ ] **Step 4: 写 M4 技术结论（总体通过 → 小范围测试版就绪）。**
- [ ] **Step 5: 提交** `docs: record M4 creation loop verdict`

## M4 完成定义

1. 创建闭环：上传 → 生成 1 个候选 → 预览确认（满意/重新生成/放弃）→ 编译 → 桌面可用。
2. 生成任务可进度/取消/恢复，失败不破坏已有宠物。
3. 抠图质量闸门拦截过度抠图，降级路径可用。
4. 多宠保存/切换/单宠激活。
5. 断网完全离线可用。
6. 隐私提示与照片生命周期明示。
7. 单候选流程分支可用：满意可编译上线；重新生成不重复建宠物；放弃清理宠物记录与任务产物。
8. M4 技术结论 → 小范围测试版可交付（Agent/声音/主动养成等仍为后续）。
