...
describe("MicroMotionController", () => {
  it("starts at a normalized neutral wave and stays inside the production ranges", () => {
    const controller = new MicroMotionController();

    expect(controller.update(0)).toEqual({ breath: 0, bodySway: 0 });
    for (let index = 0; index < 200; index += 1) {
      const frame = controller.update(100);
      expect(frame.breath).toBeGreaterThanOrEqual(-1);
  ...
  it("suppresses sway and weakens breath while carried", () => {
    ...
    for (let index = 0; index < 50; index += 1) {
      ...
      expect(Math.abs(frame.breath)).toBeLessThanOrEqual(0.25);
    }
  });
});
