...
///
/// ⚠️ **多动作包更大**（2026-09-14 起一个包里除了 idle 还会并进 yawn / lick /
/// grab-release）：idle 288 帧 ≈ 9.7 MB，加三支动作（约 120+120+81 帧）约 **20 MB**。
/// 64 MiB 对它有约 3.2 倍余量，**这个常量暂时不用动**；但哪天再往包里加动作，
/// 先按帧数估一遍再决定，别让它变成那个「偶发假故障」。
