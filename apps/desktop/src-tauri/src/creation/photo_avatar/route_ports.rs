...
//!
//! ⚠️ 这一层**只做分派，不做判断**：没有 run（`Ok(None)`）时按**默认产线**（写实风）走，
//! 与 `begin` 没给 route 时的默认保持同一个口径（`DEFAULT_PHOTO_AVATAR_ROUTE`）。
//!
...
use super::domain::{PhotoAvatarRoute, DEFAULT_PHOTO_AVATAR_ROUTE};
...
impl RoutePhotoAvatarPorts {
    ...
    fn route(&self, session_id: &str) -> Result<PhotoAvatarRoute, String> {
        Ok(self
            ...
            .unwrap_or(DEFAULT_PHOTO_AVATAR_ROUTE))
    }
